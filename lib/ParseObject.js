...
export default class ParseObject {
    ...
    getTargetFileBuff () {
        ...
        if (this.level === -1) {
            ...
            let targetFilePath = path.join(parentDir.getRealPathSync(), this.text + '.tex');
            ...
            return TextBuffer.loadSync(targetFilePath);
                ...
                */
                /*
                // 'include': no absolute path, cannot read parents, cannot nest, cannot have '.tex'
                if (this.level === -1) {
                    let targetFile;
                    if (path.isAbsolute(this.text))
                        targetFile = new File(this.text + '.tex');
                    else {
                        let fileNow = new File(this.buffer.getPath());
                        targetFile = new File(path.join(fileNow.getParent().getRealPathSync(), this.text + '.tex'));
                    }
                    if (targetFile.existsSync()) {
                        return {
                            text: targetFile.getRealPathSync(),
                            level: 1,
                        ...
                        return {
                            text: targetFile.getRealPathSync() + ' err',
                            level: 1,
                            startPt: new Point(this.matchStartPt.row, this.matchStartPt.column)
                        }
                    }
